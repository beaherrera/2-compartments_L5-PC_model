...
stochastic = true;  % if true system is stochastic - if false system is
% deterministic
...
Rd_i = 65e6; % [Ohm] transfer resistance between the Soma/basal-dendrites
% and Trunk/apical-dendrites compartments

I_trans = zeros(length(tspan),unit_number); % transfer current between 
% soma and apical tuft per neuron
ICaL = zeros(length(tspan),unit_number); % Ca2+ current per neuron

rng('shuffle')

fileL5PCS = ['data' num2str(unit_number) 'n']; % name of the folder where
...
pulse_train = 0; % if true(=1): the somatic/dendritic input is a train of
% square current pulses
noisy_current = 0; % if true(=1): the stimuli is a noisy current with 
...
if stochastic
    SigX = [g_Vs, g_Vd, g_NamS, g_NahS, g_KmS, g_CamD, g_Cai,...
        g_NapmD, g_NaphD, g_KsmD, g_KshD, g_ImmD]'./sqrt(dt);
else
...
if const_current
    ...
    % --------- Soma, input current
    I3_amp = 1e-6;% [mA] amplitude of the somatic input I3; we used 
    % I3_amp=1e-6 mA in the paper
    ...
    % generated by a double exponential function of the form
    % I2_amp*(1-exp(-1/tau1))*exp(-1/tau2) 
    % with tau1=2 ms and tau2=10 ms.
    % When I2_stim_end=0, the code selects the double exponential as input
    % current.
    I2_amp = 0.6e-6; % [mA] amplitude of the dendritic input I2; threshold
    % for Ca-spike=~0.805e-6 mA for the double exponential case
    ...
    I_parameters = [I3_amp I3_stim_onset I3_stim_end I2_amp I2_stim_onset I2_stim_end]; % 
    % to enter non-state variables into the function
...
elseif pulse_train
    ...
    I3_sq = square_pulse(I3_amp, width_I3, IP_I3, NC_I3, len, PWindowI3); % 
    % generates the square pulses
    ...
    I2_sq = square_pulse(I2_amp, width_I2, IP_I2, NC_I2, len, PWindowI2); % 
    % generates the square pulses
...
elseif noisy_current
    ...
    % --------- Soma, input current
    I3_mu = 90e-6; % effective current amplitude. This amplitude is multiply
    % by randn resulting in a noisy current with amplitudes between -5 and 5nA.
...
Vd = Xs(:,ii*unit_number+1:(ii+1)*unit_number);% ii=ii+1;  % [mV] membrane
% potential at the tuft dendrites PC
% Ca_i = Xs(1:CSR:end, ...
%     6*unit_number+1:(6+1)*unit_number) + stdInstCa.*randn(size(Xs(1:CSR:end, ...
%     7),1),1); % intracellular Ca2+ concentration, it's downsampled
...
%% Visualization
if trial<=2
    figure; plot(ts, Vs, 'k', 'LineWidth', 1); 
...
file = fullfile('..', 'GenerateFigures', 'FiguresData', fileL5PCS);
if ~exist(file, 'dir') % checks if the folder already exists
    mkdir(file); 
end

save(fullfile(file, ['voltageNoIhTrial' num2str(trial) '.mat']), 'voltage', ...
    '-v7.3');
save(fullfile(file, ['currentsNoIhTrial' num2str(trial) '.mat']), 'Ysd', ...
    'I_trans', '-v7.3');
save(fullfile(file, ['ICaLNoIhTrial' num2str(trial) '.mat']), 'ICaL', ...
    '-v7.3');
...
[lfp, lfpT] = lfp_calculation(tf, 0, ntrials, fileL5PCS);

%% Computing the CSD

iCSD_Pettersen(tf, 0, lfp, lfpT, fileL5PCS)
