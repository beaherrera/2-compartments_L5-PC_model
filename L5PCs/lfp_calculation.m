...
for trial=1:ntrials
    ...
    if Ih
        load(fullfile(file, ['currentsTrial' num2str(trial) '.mat']), 'Ysd');
    else
        load(fullfile(file, ['currentsNoIhTrial' num2str(trial) '.mat']), 'Ysd');
    end
    ...
    sources_number = length(Ids(1,:)); % number of sources in the column 
    % (all neurons)
    ...
    a = 1025;
    b = 1450;
    rNum = rand(1,unit_number); % generating the random numbers for the
    % z-direction
    ii = 1;
    AISZ = (b-a).*rNum + a; % this calculates the z-coordinate of the axon 
    % initial segment of each neuron
    zc(1,ii:unit_number) = (1000-700).*rNum + 700; % this calculates the
    % z-coordinate of the soma
    zc(1,ii*unit_number+1:(ii+1)*unit_number) = AISZ; ii = ii+1; % initial
    % segment source position
    zc(1,ii*unit_number+1:(ii+1)*unit_number) = AISZ + 150; ii = ii+1; % 
    % basal compartment source position, 150 um below the axon initial segment
    zc(1,ii*unit_number+1:(ii+1)*unit_number) = AISZ - ls; ii = ii+1; % 
    % main-bifurcation point, ls um above the soma
    zc(1,ii*unit_number+1:(ii+1)*unit_number) = AISZ - ls - 150; % tuft, 
    % 150 um above the main-bifurcation point
    ...
    ze = a:elec_spacing:((Ne-1)*elec_spacing + a); % position of the 
    % electrodes along z with respect to pia matter
    el_pos = ze*1e-3;  % [m] electrode positions with respect to the 
    % pia surface
    ...
    %% LFP calculation
    if trial==1
        lfpT = zeros(length(ze), length(Iz(1,:)), ntrials); % preallocating
        % the local field potential matrix
    end
    ...
    for ii=1:Ne
        [lfpt(ii,:), ~] = lfp_pointSourceMethod(Iz, xc, yc, zc, cond, ...
            el_pos(ii), elec_spacing.*1e-3); % mV
    end
...
if Ih % Ih present
    save(fullfile(file, 'lfp_L5PCs.mat'), 'lfp');
    save(fullfile(file, 'lfp_L5PCsTrials.mat'), 'lfpT');
else % Ih blocked
    save(fullfile(file, 'lfp_L5PCsNoIh.mat'), 'lfp');
    save(fullfile(file, 'lfp_L5PCsNoIhTrials.mat'), 'lfpT');
end
