...
Is = ((alpha).*(dVsdt.*Cm_s-(-31.5-Vs)./R_m) + a.*IKdr - I3); % soma/top oblique dendrites
IsDx= (INa); % axon intial segment 
Ib = ((1-alpha).*(dVsdt.*Cm_s-(-31.5-Vs)./R_m) + (1-a).*IKdr); % basal dendrites
Imbd = ICa + (beta).*IKs; % main bifurcation apical dendrites
Itd = (Ih + Im + INap - I2  + (1-beta).*IKs + (dVddt.*Cm_d) - (-48.1-Vd)./Rd_m); % tuft dendrites 
