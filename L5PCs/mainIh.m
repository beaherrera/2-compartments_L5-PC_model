...
stochastic = true;  % if true system is stochastic - if false system
% is deterministic
...
I_trans = zeros(length(tspan),unit_number); % transfer current between soma
% and apical tuft per neuron
...
X0 = [Vs_0, Vd_0, NamS_0, NahS_0, KmS_0, CamD_0, Cai_0, ...
    NapmD_0, NaphD_0, KsmD_0, KshD_0, Ihm_0, Imm_0]';
...
if stochastic
    SigX = [g_Vs, g_Vd, g_NamS, g_NahS, g_KmS, g_CamD, g_Cai,...
        g_NapmD, g_NaphD, g_KsmD, g_KshD, g_IhmD, g_ImmD]'./sqrt(dt);
else
...
if const_current
    ...
    % current.
    I2_amp = 0.6e-6; % [mA] amplitude of the dendritic input I2; 
    % threshold for Ca-spike=~0.805e-6 mA for the double exponential case
...
elseif noisy_current
    ...
    % --------- Soma, input current
    I3_mu = 90e-6; % effective current amplitude. This amplitude is 
    % multiply by randn resulting in a noisy current with amplitudes
    % between -5 and 5nA.
...
% Ca_i = Xs(1:CSR:end, ...
%     7*unit_number+1:(7+1)*unit_number) + stdInstCa.*randn(size(Xs(1:CSR:end, ...
...
save(fullfile(file, ['voltageTrial' num2str(trial) '.mat']), 'voltage', ...
    '-v7.3');
save(fullfile(file, ['currentsTrial' num2str(trial) '.mat']), 'Ysd', ...
    'I_trans', '-v7.3');
save(fullfile(file, ['ICaLTrial' num2str(trial) '.mat']), 'ICaL', ...
    '-v7.3');
