...
% stimulus type
const_current = 1; % if true(=1): soma stimuli is a current step pulse, 
% and the dendritic stimuli is either a current step pulse or a double exponential
...
if const_current % save the simulated data for the BAC-firing paradigm
    ...
    save(fullfile(fileB, ['voltagenoIh' num2str(paradigm) '.mat']), ...
        'Vs', 'Vd');
    save(fullfile(fileB, ['stimulusnoIh' num2str(paradigm) '.mat']), ...
        'I3_Final', 'I2_Final');
...
elseif pulse_train % save the simulated data for the critical frequency study
    ...
    save(fullfile(fileCF, 'voltagenoIhFreq.mat'), 'Vsf', 'Vdf', '-v7.3');
    save(fullfile(fileCF, 'stimulusnoIhFreq.mat'), 'I3f', ...
        'I2_Final', '-v7.3');
    save(fullfile(fileCF, 'calciumnoIhFreq.mat'), 'Ca_if');
    ...
    if nnz(I2_sq) ~= length(I2_sq) % True for somatic stimulation
        save(fullfile(fileR, 'voltagenoIhRamp.mat'), 'VsR', 'VdR', '-v7.3');
        save(fullfile(fileR, 'stimulusnoIhRamp.mat'), 'I3_Final', ...
            'mean_u', '-v7.3');
    else % True for dendritic stimulation
        save(fullfile(fileR, 'voltagenoIhRampD.mat'), 'VsR', ...
            'VdR', '-v7.3');
        save(fullfile(fileR, 'stimulusnoIhRampD.mat'), 'I2_Final',...
        'mean_u', '-v7.3');
    end
end
