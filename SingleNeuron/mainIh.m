...
if const_current
    ...
    % current.
    I2_amp = 0.9e-6; % [mA] amplitude of the dendritic input I2;
    % threshold for Ca-spike=~0.7637 mA for the double exponential case
    I2_stim_onset = 37e-3; % [s] stimulus onset; dt = 2 ms for
    % somatic-dendritic coupling
    I2_stim_end = 0; % [s] stimulus end; if this var is equal to zero,
    % a EPSP-like stimuli is generated
...
VshD = 8; % [mV] Shift in the dendritic channels kinetics to account for the
% shift in the dendritic resting potential
...
if stochastic
    SigX = [g_Vs, g_Vd, g_NamS, g_NahS, g_KmS, g_CamD, g_Cai, g_NapmD,...
        g_NaphD, g_KsmD, g_KshD, g_IhmD, g_ImmD]'./sqrt(dt);
else
...
Ca_i = Xs(1:CSR:end,7) + stdInstCa.*randn(size(Xs(1:CSR:end,7),1),1); % 
% intracellular Ca2+ concentration, it's downsampled
...
file = fullfile('..', 'GenerateFigures', 'FiguresData');
...
if const_current  % save the simulated data for the BAC-firing paradigm
    fileB = fullfile(file, 'BACfiring');
    if ~exist(fileB, 'dir') % checks if the folder already exists
        mkdir(fileB); % creates a folder named 'fileB'
    end
    
    save(fullfile(fileB, ['voltageIh' num2str(paradigm) '.mat']), ...
        'Vs', 'Vd');
    save(fullfile(fileB, ['stimulusIh' num2str(paradigm) '.mat']), ...
        'I3_Final', 'I2_Final');
    
elseif pulse_train % save the simulated data for the critical frequency study
    
    fileCF = fullfile(file, 'CF');
    if ~exist(fileCF, 'dir') % checks if the folder already exists
        mkdir(fileCF); % creates a folder named 'fileCF'
    end
    
    save(fullfile(fileCF, 'voltageIhFreq.mat'), 'Vsf', 'Vdf', '-v7.3');
    save(fullfile(fileCF, 'stimulusIhFreq.mat'), 'I3f', ...
        'I2_Final', '-v7.3');
    save(fullfile(fileCF, 'calciumIhFreq.mat'), 'Ca_if');
    
else % save the simulated data for a ramp input current
    fileR = fullfile(file, 'CurrentRamp');
    ...
    if nnz(I2_sq) ~= length(I2_sq) % True for somatic stimulation
        save(fullfile(fileR, 'voltageIhRamp.mat'), 'VsR', 'VdR', '-v7.3');
        save(fullfile(fileR, 'stimulusIhRamp.mat'), 'I3_Final', ...
            'mean_u', '-v7.3');
    else % True for dendritic stimulation
        save(fullfile(fileR, 'voltageIhRampD.mat'), 'VsR', ...
            'VdR', '-v7.3');
        save(fullfile(fileR, 'stimulusIhRampD.mat'), 'I2_Final',...
        'mean_u', '-v7.3');
    end
end
